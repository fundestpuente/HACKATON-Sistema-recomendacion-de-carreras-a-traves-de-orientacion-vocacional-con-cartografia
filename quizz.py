...
def cargar_datos():
    try:
        ...
        # 2. Normalizar nombres de columnas (Para evitar el KeyError)
        df.columns = df.columns.str.lower().str.strip()
        
        # Mapa de renombrado para asegurar que existan las columnas clave
        renames = {
            ...
            'carrera_senescyt': 'carrera'
        }
        df = df.rename(columns=renames)
        
        # 3. Validación de seguridad
        required_cols = ['universidad', 'provincia', 'carrera']
        missing = [c for c in required_cols if c not in df.columns]
        
        if missing:
            st.error(f"⚠️ El archivo CSV no tiene las columnas esperadas. Faltan: {missing}. Columnas encontradas: {list(df.columns)}")
            return pd.DataFrame()

        # 4. Limpieza de datos (Ahora seguro porque las columnas existen)
...
def obtener_universidades(df, keywords):
    ...
    if 'universidad' not in df_trabajo.columns:
        # Intentar normalizar si viene de fuera
        df_trabajo.columns = df_trabajo.columns.str.lower().str.strip()
        renames = {'nombre_ies': 'universidad', 'nombre_provincia': 'provincia', 'nombre_carrera': 'carrera'}
        df_trabajo = df_trabajo.rename(columns=renames)

    # Asegurar tipos string y limpieza básica
    for col in ['carrera', 'universidad', 'provincia']:
        if col in df_trabajo.columns:
            df_trabajo[col] = df_trabajo[col].astype(str).str.strip()

    mask = df_trabajo['carrera'].str.lower().apply(
        lambda x: any(keyword.lower() in x for keyword in keywords)
    )
    carreras_filtradas = df_trabajo[mask]
